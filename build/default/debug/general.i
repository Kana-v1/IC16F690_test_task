...
static unsigned long calculate_timer_counts(float us_to_wait) {
   float secs_to_wait = us_to_wait / 10000000;

    float timer_counts = (float)((4000000UL))/(secs_to_wait * 1) - 1;
    unsigned long counts_to_wait = (unsigned long)(0xFFFF - timer_counts);
...
void sleep_us_thread1(unsigned int time) {
    ...
    unsigned long timer_counts = calculate_timer_counts(time);
    TMR1H = (timer_counts >> 8) & 0xFF;
    TMR1L = timer_counts & 0xFF;
    INTCONbits.GIE = 1;
    T1CONbits.TMR1ON = 1;
